...
X_cols = ['Type','Air temperature [K]', 'Process temperature [K]','Rotational speed [rpm]', 'Torque [Nm]', 'Tool wear [min]','air_process_diff','power', 'overstrain']

y_cols = ['Machine failure', 'TWF', 'HDF', 'PWF', 'OSF','RNF' ]
...
train_acc2 = round(rfc.score(X_train, y_train) * 100, 1)
...
#final=[np.array(inputt)]
#b = svc.predict(final)
rfcFile = open('RandomForestModel.pkl', 'wb')
pickle.dump(rfc, rfcFile)
rfcFile.close()
